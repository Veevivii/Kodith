...
**Currently disabled** — the first design (a pixel-grid rabbit-with-a-cap)
didn't land, so it's turned off pending a better one. The code is intact,
just switched off, so dropping in a new design is a swap, not a rebuild.

It has three placements, all the same `<svg>` markup kept in sync by hand:

- beside the "Join Kodith" button in the hero — commented out in `index.html`
- beside the Discord/Instagram buttons in the Join section — also commented
  out in `index.html`
- next to the "nothing on the calendar" message when Upcoming is empty,
  gated behind `MASCOT_ENABLED` in `js/main.js` (that spot is conditional, so
  it has to be JS-rendered rather than commented HTML)

**To bring back the current design:** delete the two `<!-- -->` comment
wrappers in `index.html` and flip `MASCOT_ENABLED = true` in `js/main.js`.

**To replace it with a new design:** update the `MASCOT_SVG` constant in
`js/main.js` first (it has the clearest comments on the pixel grid), then
copy the same `<svg>...</svg>` into the two commented-out spots in
`index.html`, then do the enable step above.
...
that part needs no changes when the design changes.
